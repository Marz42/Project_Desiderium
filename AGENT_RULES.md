...
- `docs/rfc/`：本项目的提案/RFC 文档区，也按 OKF concept 文档维护。
...
- `memory-bank/knowledge/plans/*.md` — 中期计划（多会话、多任务的路标）。
- `memory-bank/knowledge/contracts/*.md` — API、数据库、工具、仓库契约。
- `memory-bank/knowledge/manuals/*.md` — 操作手册。
- `docs/rfc/*.md` — 提案与演进设计。
- **`DESIGN.md`（项目根目录）** — 视觉设计规范（若存在）。涉及前端/UI 开发时作为 WARM 参考；格式说明见 `memory-bank-template/DESIGN.md` 模板。
...
5. 若任务涉及前端/UI 且项目根目录存在 `DESIGN.md`，将其加入读取列表。
